...
THIS_SERVICE_URI = ENV['DOCKERCLOUD_SERVICE_API_URI']
THIS_STACK = ENV['DOCKERCLOUD_STACK_NAME']
...
EM.run {
  ...
  def init_nginx_config
    ...
    @my_services = HttpServices.new(ENV['DOCKERCLOUD_AUTH'], RESTRICT_MODE, THIS_NODE, THIS_SERVICE_URI).write_conf(ENV['NGINX_DEFAULT_CONF'])
    HttpServices.reload!
  end
